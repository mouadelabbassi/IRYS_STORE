...
public interface ProductRepository extends JpaRepository<Product, String>, JpaSpecificationExecutor<Product> {
    ...
    Optional<Product> findByAsinAndApprovalStatus(String asin, Product.ApprovalStatus status);

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT p FROM Product p WHERE p.asin = :asin")
    Optional<Product> findByAsinForUpdate(@Param("asin") String asin);

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT p FROM Product p WHERE p.asin IN :asins AND p.approvalStatus = :status ORDER BY p.asin")
    List<Product> findAllApprovedByAsinForUpdate(
            @Param("asins") List<String> asins,
            @Param("status") Product.ApprovalStatus status
    );
    ...
    Page<Product> findByStockQuantityGreaterThanAndStockQuantityLessThan(
            Integer lowerBound,
            Integer upperBound,
            Pageable pageable
    );
