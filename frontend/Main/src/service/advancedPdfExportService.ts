...
const COLORS = {
    primary: [155, 106, 20] as [number, number, number],
    primaryDark: [104, 65, 13] as [number, number, number],
...
export const generateAdvancedPDF = async (): Promise<void> => {
    ...
    doc.setFontSize(42);
    doc.setFont('helvetica', 'bold');
    doc.text('Irys Store', pageWidth / 2, 80, { align: 'center' });
    ...
    doc.setFontSize(8);
    doc.setTextColor(...COLORS.gray);
    doc.text(`Irys Store Analytics Report | Generated: ${new Date().toLocaleDateString()}`, pageWidth / 2, pageHeight - 10, { align: 'center' });
    ...
    doc.setFontSize(18);
    doc.setFont('helvetica', 'bold');
    doc.text('Irys Store', pageWidth / 2, pageHeight - 35, { align: 'center' });
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    ...
    const filename = `Irys_Store_Executive_Report_${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(filename);
