...
export const generateAdminPDF = async (data: {
    ...
    // Colors
    const primaryColor:  [number, number, number] = [155, 106, 20];
    const secondaryColor: [number, number, number] = [39, 39, 42];
    ...
    const addFooter = (pageNum: number) => {
        ...
        doc.text(
            `Irys Store Admin Report - Page ${pageNum}`,
            pageWidth / 2,
            pageHeight - 10,
            { align: 'center' }
        );
    ...
    // Save
    doc.save(`Irys_Store_Admin_Report_${new Date().toISOString().split('T')[0]}.pdf`);
};
...
export const generateAnalystPDF = async (data: {
    ...
    doc.setFontSize(36);
    doc.setFont('helvetica', 'bold');
    doc.text('Irys Store', pageWidth / 2, 100, { align: 'center' });
    ...
    doc.setTextColor(...grayColor);
    doc.text(
        `Irys Store Analytics Report - Generated ${new Date().toLocaleDateString()}`,
        pageWidth / 2,
        pageHeight - 10,
        { align: 'center' }
    );
    ...
    // Save
    doc.save(`Irys_Store_Analytics_Report_${new Date().toISOString().split('T')[0]}.pdf`);
};
