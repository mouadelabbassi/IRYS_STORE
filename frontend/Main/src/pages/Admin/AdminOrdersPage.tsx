...
const AdminOrdersPage: React.FC = () => {
    ...
    const handleStatusChange = async (orderId: number, action: 'confirm' | 'cancel', e: React.MouseEvent) => {
        e.stopPropagation();
        ...
        try {
            ...
            setReceiptOrder(current => current?.id === orderId ? updatedOrder : current);
        } catch (error) {
            ...
            <head>
                ...
                <style>
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    ...
                    .logo { font-size: 24px; font-weight: bold; color: #9b6a14; }
...
Phone: ${receiptOrder.customerPhone}
Address: ${receiptOrder.shippingAddress}
City: ${receiptOrder.shippingCity}
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500 dark:text-gray-400">
                                                {order.userName} • {order.customerPhone} • {order.shippingCity}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        ...
                                        <button
                                            ...
                                            className="p-2 text-brand-600 hover:bg-brand-100 dark:text-brand-400 dark:hover:bg-brand-900/30 rounded-lg transition-colors"
                                            title="View Receipt"
                                        >
                                    ...
                                    <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {order.status === 'PENDING' && (
                                                <button
                                                    onClick={(e) => handleStatusChange(order.id, 'confirm', e)}
                                                    disabled={updatingOrderId === order.id}
                                                    className="px-3 py-1.5 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white text-sm font-medium rounded-lg"
                                                >
                                                    Confirm order
                                                </button>
                                            )}
                                            {order.status === 'PENDING' && (
                                                <button
                                                    onClick={(e) => handleStatusChange(order.id, 'cancel', e)}
                                                    disabled={updatingOrderId === order.id}
                                                    className="px-3 py-1.5 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white text-sm font-medium rounded-lg"
                                                >
                                                    Cancel order
                                                </button>
                                            )}
                                            <button
                                                onClick={(e) => handleViewReceipt(order, e)}
                                                className="flex items-center gap-2 px-3 py-1.5 bg-brand-600 hover:bg-brand-700 text-white text-sm font-medium rounded-lg transition-colors"
                                            >
                                                View Receipt
                                            </button>
                                        </div>
                                    </div>
                                    <div className="mb-4 rounded-lg border border-gray-200 bg-white p-3 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-hidden">
                        ...
                        <div className="overflow-y-auto max-h-[60vh] p-6" ref={receiptRef}>
                            <div className="text-center mb-6 pb-6 border-b-2 border-dashed border-gray-300 dark:border-gray-600">
                                ...
                                <p className="text-gray-500 dark:text-gray-400 text-sm">www.irysstore.com</p>
                            </div>
                            <div className="mb-6">
                            ...
                            <div className="mb-6 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                ...
                                <p className="text-sm text-gray-500 dark:text-gray-400">{receiptOrder.customerPhone}</p>
                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                    {receiptOrder.shippingAddress}, {receiptOrder.shippingCity}
                                </p>
                            </div>
                            ...
                            <div className="mt-6 pt-6 border-t-2 border-dashed border-gray-300 dark:border-gray-600 text-center">
                                ...
                                <p className="text-gray-400 dark:text-gray-500 text-xs mt-2">
                                    Questions? Contact support@irysstore.com
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3 p-4 border-t dark:border-gray-700 bg-gray-50 dark: bg-gray-900/50">
                            ...
                            <button
                                ...
                                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-brand-600 hover:bg-brand-700 text-white font-medium rounded-lg transition-colors"
                            >
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
